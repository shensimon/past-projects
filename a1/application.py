...
def process_event_history(log: Dict[str, List[Dict]],
    ...
    for event_data in log['events']:
        curr = datetime.datetime.strptime(event_data['time'],
                                          '%Y-%m-%d %H:%M:%S')
        if curr.month != billing_month:
            new_month(customer_list, billing_date.month, billing_date.year)
            new_month(customer_list, curr.month, curr.year)
            billing_month = billing_date.month
        if event_data['type'] == 'call':
            ...
            cust1 = find_customer_by_number(event_data['src_number'],
                                            customer_list)
            cust1.make_call(call)
            cust2 = find_customer_by_number(event_data['dst_number'],
                                            customer_list)
            cust2.receive_call(call)
