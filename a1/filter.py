    ...
    def apply(self, customers: List[Customer],
        ...
        for c in customers:
            if c.get_id() == int(filter_string):
                cnums = c.get_phone_numbers()
                for call in data:
                    if call.src_number in cnums or call.dst_number in cnums:
...
class LocationFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        if len(coord) != 4:
            return data
        if float(coord[0]) < -79.697878 or float(coord[1]) < 43.576959 or \
                float(coord[2]) > -79.196382 or float(coord[3]) > 43.799568:
            return data
        if float(coord[0]) > float(coord[2]) or float(coord[1]) > \
                float(coord[3]):
            return data
        filtered_list = []
        for call in data:
            if float(coord[0]) < call.src_loc[0] < float(coord[2]) and \
                    float(coord[1]) < call.src_loc[1] < float(coord[3]):
                filtered_list.append(call)
            if float(coord[0]) < call.dst_loc[0] < float(coord[2]) and \
                    float(coord[1]) < call.dst_loc[1] < float(coord[3]):
                filtered_list.append(call)
