...
class Election:
    ...
    def update_results(self, riding: str, party: str, votes: int) -> None:
        ...
        1001
        """
        if riding not in self.ridings:
            self.ridings.append(riding)
        if party not in self.parties:
            self.parties.append(party)
        if riding not in self.results:
            self.results[riding] = {party: votes}
        elif riding in self.results:
            if party not in self.results[riding]:
                self.results[riding][party] = votes
            elif party in self.results[riding]:
                self.results[riding][party] += votes
    ...
    def read_results(self, instream: IO[str]) -> None:
        ...
        for line in instream:
            data = line.split(',')
            self.update_results(data[1].replace('"', ''),
                                data[13].replace('"', ''),
                                int(data[17][:-1]))
    ...
    def results_for(self, riding: str, party: str) -> Optional[int]:
        ...
        1
        """
        if riding not in self.results or party not in self.results[riding]:
            return None
        else:
            return self.results[riding][party]
    ...
    def riding_winners(self, riding: str) -> List[str]:
        ...
        winner = []
        votes = 0
        for party in self.results[riding]:
            if self.results[riding][party] > votes:
                votes = self.results[riding][party]
                winner = [party]
            elif self.results[riding][party] == votes:
                winner.append(party)
        return winner
    ...
    def popular_vote(self) -> Dict[str, int]:
        ...
        for riding in self.ridings:
            for party in self.parties:
                if party not in popular and party in self.results[riding]:
                    popular[party] = self.results[riding][party]
                elif party in popular and party in self.results[riding]:
                    popular[party] += self.results[riding][party]
        return popular
    ...
    def party_seats(self) -> Dict[str, int]:
        ...
        for riding in self.ridings:
            if len(self.riding_winners(riding)) == 1:
                seats[self.riding_winners(riding)[0]] += 1
        return seats
    ...
    def election_winners(self) -> List[str]:
        ...
        for party in self.party_seats():
            if self.party_seats()[party] > seats:
                winner = [party]
                seats = self.party_seats()[party]
            elif self.party_seats()[party] == seats:
                winner.append(party)
        return winner
...
class Jurisdiction:
    ...
    def read_results(self, year: int, month: int, day: int, instream: IO[str]) \
        ...
        if date(year, month, day) not in self._history:
    ...
    def party_wins(self, party: str) -> List[date]:
        ...
        for time in self._history:
            if self._history[time].election_winners() == [party]:
                wins.append(time)
        return wins
    ...
    def party_history(self, party: str) -> Dict[date, float]:
        ...
        for time in self._history:
            total = 0
            for group in self._history[time].popular_vote():
                total += self._history[time].popular_vote()[group]
            ratio = self._history[time].popular_vote()[party]/total
            phistory[time] = ratio
        return phistory
    ...
    def riding_changes(self) -> List[Tuple[Set[str], Set[str]]]:
        ...
        pass
        # for time in self._history:
        #     first = self._history[time].ridings_of()
