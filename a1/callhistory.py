...
class CallHistory:
    ...
    def register_outgoing_call(self, call: Call) -> None:
        ...
        """
        date_m = call.time.month
        date_y = call.time.year
        if (date_m, date_y) not in self.outgoing_calls:
            self.outgoing_calls[(date_m, date_y)] = [call]
        elif (date_m, date_y) in self.outgoing_calls:
            self.outgoing_calls[(date_m, date_y)].append(call)
    ...
    def register_incoming_call(self, call: Call) -> None:
        ...
        date_m = call.time.month
        date_y = call.time.year
        if (date_m, date_y) not in self.incoming_calls:
            self.incoming_calls[(date_m, date_y)] = [call]
        elif (date_m, date_y) in self.incoming_calls:
            self.incoming_calls[(date_m, date_y)].append(call)
